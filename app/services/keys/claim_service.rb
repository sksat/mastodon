...
class Keys::ClaimService < BaseService
  ...
  def claim_remote_key!
    query_result = QueryService.new.call(@account)
    ...
    @result = Result.new(@account, @device_id, key_id: json['id'], key: json['publicKeyBase64'], signature: json.dig('signature', 'signatureValue'))
  end
  ...
  def build_post_request(uri)
    Request.new(:post, uri).tap do |request|
      request.on_behalf_of(Account.representative, :uri)
